...
import getLawyerAi from "../model/LawyerAI.js";
        ...
        if (validarCPF(cpfOab)) {
            ...
            if (userInfo.cpf === cpfOab) {
                if(userInfo.senha === senha){
                    var infoUser = localStorage.setItem('loggedInUser', JSON.stringify(userInfo));
                    console.log('Informações do usuário salvas no localStorage:', userInfo);
                    if (infoUser){
                        let userAI = getLawyerAi(infoUser);
                        console.log("Advogado AI:", userAI);
                        return userAI;
                    }
            ...
            if (userInfo.oab === cpfOab) {
                if(userInfo.senha === senha){
                    var infoAdv = localStorage.setItem('loggedInUser', JSON.stringify(userInfo));
                    console.log('Informações do usuário salvas no localStorage:', userInfo);
                    if (infoAdv){
                        let advAI = getLawyerAi(infoAdv);
                        console.log("Advogado AI:", advAI);
                        return advAI;
                    }
